import pimg1 from '/public/images/project/img-1.jpg'
import pimg2 from '/public/images/project/img-2.jpg'
import pimg3 from '/public/images/project/img-3.jpg'
import pimg4 from '/public/images/project/img-4.jpg'
import pimg5 from '/public/images/project/img-5.jpg'
import pimg6 from '/public/images/project/img-6.jpg'

import ps1img1 from '/public/images/project-single/1.jpg'
import ps1img2 from '/public/images/project-single/4.jpg'
import ps1img3 from '/public/images/project-single/5.jpg'
import ps1img4 from '/public/images/project-single/6.jpg'
import ps1img5 from '/public/images/project-single/7.jpg'
import ps1img6 from '/public/images/project-single/8.jpg'

import psub1img1 from '/public/images/project-single/2.jpg'
import psub1img2 from '/public/images/project-single/3.jpg'


const Projects = [
    {
       Id:'1',
       pImg:pimg1,
       ps1img:ps1img1,
       psub1img1:psub1img1,
       psub1img2:psub1img2,
       title:'Playtaunt',  
       subTitle:'Web App',
       tech :  'React.js, JavaScript, React-Bootstrap, CSS, Node.js, Web3.js, Polygon',
    },
    {
       Id:'2',
       pImg:pimg2,
       ps1img:ps1img2,
       psub1img1:psub1img1,
       psub1img2:psub1img2,
       title:'Wagmigame', 
       subTitle:'Web App',
       tech :  'Angular, JavaScript, Tailwind CSS, C#, Ether.js, Ethereum, Binance',
    },
    {
      Id:'3',
      pImg:pimg3,
      ps1img:ps1img3,
      psub1img1:psub1img1,
      psub1img2:psub1img2,
      title:'Bang-olufsen',  
      subTitle:'Web Development',   
      tech :  'React.js, Next.js, TypeScript, Styled-component, Node.js, Video.js, Azure',
   },
   {
      Id:'4',
      pImg:pimg4,
      ps1img:ps1img4,
      psub1img1:psub1img1,
      psub1img2:psub1img2,
      title:'fcma', 
      subTitle:'Web Development',  
      tech :  'Vue.js, JavaScript, CSS3, Laravel, AWS, MySQL',
   },
   {
      Id:'5',
      pImg:pimg5,
      ps1img:ps1img5,
      psub1img1:psub1img1,
      psub1img2:psub1img2,
      title:'moen', 
      subTitle:'Web & Mobile',
      tech : 'Angular, Typescript, CSS, SCSS, jQuery, GCP, Node.js, React Native '    
   },
   {
      Id:'6',
      pImg:pimg6,
      ps1img:ps1img6,
      psub1img1:psub1img1,
      psub1img2:psub1img2,
      title:'winwinx', 
      subTitle:'Web & Mobile App',    
      tech : 'React, Next.js, CSS, Node.js, HubSpot, AWS, React Native '    
   },
]

export default Projects;